var U = {
    ...
    reduce: function(arr,fn,initial,context){var acc=(initial===undefined)?0:initial;for(var i=0;i<arr.length;i++){acc=fn.call(context,acc,arr[i],i,arr);}return acc;},
    ...
    strPadStart: function(len,str){return String.prototype.padStart(len,str);},
    strPadEnd: function(len,str){return String.prototype.padEnd(len,str);},
    ...
    strToHTMLText: function(str,revStart,revEnd){var map={'&':'amp','\"':'quot','<':'lt','>':'gt'};str=str.replace(/(&|"|<|>)/g,function(match,k){return('&'+map[k]+';')||k;});str=str.replace(/(\S)(\s+)(\S)/g,function(match,start,spaces,end){var len=spaces.split('').length;var s=start;for(var i=0;i<len;i++){s+=(i%2==0)?' ':'&nbsp;';}return s+end;});str=str.replace(/^\s+/,function(spaces){var len=spaces.split('').length;var arr=[];for(var i=0;i<len;i++){arr[i]=(i%2==0)?(revStart?' ':'&nbsp;'):(revStart?'&nbsp;':' ');}return revStart?arr.reverse().join(''):arr.join('');});str=str.replace(/\s+$/,function(spaces){var len=spaces.split('').length;var arr=[];for(var i=0;i<len;i++){arr[i]=(i%2==0)?(revEnd?' ':'&nbsp;'):(revEnd?'&nbsp;':' ');}return revEnd?arr.join(''):arr.reverse().join('');});return str;},
    ...
    strUntil: function(str,splitRegex){var parts=str.split(splitRegex);return(Array.isArray(parts)&&parts[0])?parts[0]:'';},
    strReverseUntil: function(str,splitRegex){var rev='';for(var i=str.length-1;i>=0;i--){rev+=str[i];}var parts=rev.split(splitRegex);return(Array.isArray(parts)&&parts[0])?parts[0]:'';},
    regexMatchLength: function(str,regex,i){var matches=str.match(regex);i=parseInt(i);i=isNaN(i)?0:i;return(Array.isArray(matches)&&matches[i])?matches[i].length:0;},
    arrRemove: function(arr,fn,v){var isFN=typeof(fn)==='function';var isV=v!==undefined;var tmp=[];for(var i=0,length=arr.length;i<length;i++){if(isFN){!fn.call(arr,arr[i],i)&&tmp.push(arr[i]);continue;}if(isV){arr[i]&&arr[i][fn]!==v&&tmp.push(arr[i]);continue;}arr[i]!==fn&&tmp.push(arr[i]);}return tmp;},
    arrFirst: function(arr){return arr[0]||null;},
    arrLast: function(arr){return arr[arr.length-1]||null;},
    arrOrderBy: function(arr,name,asc,maxlength){var length=arr.length;if(!length||length===1){return arr;}if(typeof(name)==='boolean'){asc=name;name=undefined;}if(maxlength===undefined){maxlength=3;}if(asc===undefined){asc=true;}var type=0;var field=name?arr[0][name]:arr[0];switch(typeof(field)){case'string':type=1;break;case'number':type=2;break;case'boolean':type=3;break;default:if(!field instanceof Date&&!isNaN(field.getTime())){return arr;}type=4;break;}function _shellInsertionSort(list,length,gapSize,fn){var temp,i,j;for(i=gapSize;i<length;i+=gapSize){j=i;while(j>0&&fn(list[j-gapSize],list[j])===1){temp=list[j];list[j]=list[j-gapSize];list[j-gapSize]=temp;j-=gapSize;}}};function shellsort(arr,fn){var length=arr.length;var gapSize=Math.floor(length/2);while(gapSize){_shellInsertionSort(arr,length,gapSize,fn);gapSize=Math.floor(gapSize/2);}return arr;};shellsort(arr,function(a,b){var va=name?a[name]:a;var vb=name?b[name]:b;if(type===1){return(va&&vb)?(asc?U.strRemoveDiacritics(va.substring(0,maxlength)).localeCompare(U.strRemoveDiacritics(vb.substring(0,maxlength))):U.strRemoveDiacritics(vb.substring(0,maxlength)).localeCompare(U.strRemoveDiacritics(va.substring(0,maxlength)))):0;}else if(type===2){return va>vb?(asc?1:-1):va<vb?(asc?-1:1):0;}else if(type===3){return va===true&&vb===false?(asc?1:-1):va===false&&vb===true?(asc?-1:1):0;}else if(type===4){if(!va||!vb){return 0;}if(!va.getTime){va=new Date(va);}if(!vb.getTime){vb=new Date(vb);}var at=va.getTime();var bt=vb.getTime();return at>bt?(asc?1:-1):at<bt?(asc?-1:1):0;}return 0;});return arr;},
    ...
    ajax: function(obj){var xhr=new XMLHttpRequest();var res=null;var json='application/json';xhr.onreadystatechange=function(){if(xhr.readyState===4){if(obj.header['Accept']===json){try{res=JSON.parse(xhr.responseText);}catch(err){console.warn('Ajax: Unable to parse server response to JSON.');obj.error(null,xhr);}}else{res=xhr.responseText;}if(xhr.status===200){obj.success(res,xhr);}else{obj.error(res,xhr);}obj.always(res,xhr);}};xhr.upload.onprogress=function(state){if(state.lengthComputable){var percentage=(state.loaded/state.total)*100;obj.progress(percentage.toFixed(0));}};xhr.onerror=function(){console.warn('Ajax: Unexpected ajax error.');};xhr.open(obj.method,url,true);for(var k in obj.header){var v=obj.header[k];xhr.setRequestHeader(k,v);}xhr.send(obj.data);},
    ...
    isDomEl: function(el){return(el instanceof Node||el instanceof Element||el instanceof HTMLDocument);},
    domMatches: function(el,sel){var p=Element.prototype;var f=p.matches||p.webkitMatchesSelector||p.mozMatchesSelector||p.msMatchesSelector||function(s){return(Array.prototype.indexOf.call(document.querySelectorAll(s),this)!==-1);};return f.call(el,sel);},
    ...
    domPrev: function(sel){if(!sel){throw new Error('invalidParameter');}var els=null;if(Array.isArray(sel)){els=sel;}else if(U.isDomEl(sel)){els=[sel];}else{els=U.domFind(sel);els=Array.isArray(els)?els:[els];}var arr=[];if(Array.isArray(els)){for(var i=0;i<els.length;i++){var el=els[i];if(el){arr.push(getPrev(el));}}}return selectingOne(sel)?arr[0]:arr;function getPrev(el,psel){var els=(el.parentNode&&el.parentNode.children)?el.parentNode.children:[];var arr=[];var brk=false;for(var i=els.length-1;i>=0;i--){var sib=els[i];if(sib===el){brk=true;continue;}if(brk&&sib&&(U.domMatches(sib,psel)||!psel)){return sib;}else{continue;}}return null;}function selectingOne(sel){if(U.isDomEl(sel)){return true;}else if(typeof(sel)==='string'){var parts=sel.split(/\s+/);return(parts&&parts.length==1&&parts[0][0]=='#');}return false;}},
    domPrevAll: function(sel){if(!sel){throw new Error('invalidParameter');}var els=null;if(Array.isArray(sel)){els=sel;}else if(U.isDomEl(sel)){els=[sel];}else{els=U.domFind(sel);els=Array.isArray(els)?els:[els];}var arr=[];if(Array.isArray(els)){for(var i=0;i<els.length;i++){var el=els[i];if(el){arr.push(getPrevAll(el));}}}return selectingOne(sel)?arr[0]:arr;function getPrevAll(el){var els=(el.parentNode&&el.parentNode.children)?el.parentNode.children:[];var arr=[];for(var i=0;i<els.length;i++){var ch=els[i];if(ch===el){return arr;}arr.push(ch);}}function selectingOne(sel){if(U.isDomEl(sel)){return true;}else if(typeof(sel)==='string'){var parts=sel.split(/\s+/);return(parts&&parts.length==1&&parts[0][0]=='#');}return false;}},
    domNext: function(sel,nsel){if(!sel){throw new Error('invalidParameter');}var els=null;if(Array.isArray(sel)){els=sel;}else if(U.isDomEl(sel)){els=[sel];}else{els=U.domFind(sel);els=Array.isArray(els)?els:[els];}var arr=[];if(Array.isArray(els)){for(var i=0;i<els.length;i++){var el=els[i];if(el){arr.push(getNext(el,nsel));}}}return selectingOne(sel)?arr[0]:arr;function getNext(el,nsel){var els=(el.parentNode&&el.parentNode.children)?el.parentNode.children:[];var arr=[];var brk=false;for(var i=0;i<els.length;i++){var sib=els[i];if(sib===el){brk=true;continue;}if(brk&&sib&&(U.domMatches(sib,nsel)||!nsel)){return sib;}else{continue;}}return null;}function selectingOne(sel){if(U.isDomEl(sel)){return true;}else if(typeof(sel)==='string'){var parts=sel.split(/\s+/);return(parts&&parts.length==1&&parts[0][0]=='#');}return false;}},
    domNextAll: function(sel){if(!sel){throw new Error('invalidParameter');}var els=null;if(Array.isArray(sel)){els=sel;}else if(U.isDomEl(sel)){els=[sel];}else{els=U.domFind(sel);els=Array.isArray(els)?els:[els];}var arr=[];if(Array.isArray(els)){for(var i=0;i<els.length;i++){var el=els[i];if(el){arr.push(getNextAll(el));}}}return selectingOne(sel)?arr[0]:arr;function getNextAll(el){var els=(el.parentNode&&el.parentNode.children)?el.parentNode.children:[];var arr=[];var brk=false;for(var i=0;i<els.length;i++){var ch=els[i];if(brk){arr.push(ch);}if(ch===el){brk=true;}}return arr;}function selectingOne(sel){if(U.isDomEl(sel)){return true;}else if(typeof(sel)==='string'){var parts=sel.split(/\s+/);return(parts&&parts.length==1&&parts[0][0]=='#');}return false;}},
