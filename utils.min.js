var U = {
    ...
    strToHTMLText: function(str,leftStartWithNBPS,leftRevResult,rightStartWithNBPS,rightRevResult){var map={'&':'amp','\"':'quot','<':'lt','>':'gt'};str=str.replace(/(&|"|<|>)/g,function(match,k){return('&'+map[k]+';')||k;});str=replaceMultipleWhitespacesBetweenChars(str);str=str.replace(/^\s+/,function(spaces){var len=spaces.split('').length;return generateSpaces(len,leftStartWithNBPS,leftRevResult);});str=str.replace(/\s+$/,function(spaces){var len=spaces.split('').length;return generateSpaces(len,rightStartWithNBPS,rightRevResult);});return str;function replaceMultipleWhitespacesBetweenChars(str){var exp=/[^\s](\s+)[^\s]/g;while((match=exp.exec(str))!=null){if(Array.isArray(match)){exp.lastIndex-=1;var i=match.index+1;var len=match[1].length;var part=generateSpaces(len,false,true);str=str.substring(0,i)+part+str.substring(i+len);}}return str;}function generateSpaces(len,startWithNBPS,revResult){var arr=[];for(var i=0;i<len;i++){arr[i]=(i%2==0)?(startWithNBPS?'&nbsp;':' '):(startWithNBPS?' ':'&nbsp;');}return revResult?arr.reverse().join(''):arr.join('');};},
