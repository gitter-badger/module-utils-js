var U = {
    ...
    strToHTMLText: function(str){var map={'&':'amp','\"':'quot','<':'lt','>':'gt'};str=str.replace(/(&|"|<|>)/g,function(match,k){return('&'+map[k]+';')||k;});str=str.replace(/\S(\s+)\S/g,function(match,k){var len=k.split('').length;var s='';for(var i=0;i<len;i++){s+=(i%2==0)?' ':'&nbsp;';}return s;});str=str.replace(/(^\s+|\s+$)/g,function(match,k){var s='';for(var i=0;i<k.length;i++){s+='&nbsp;';}return s;});return str;},
